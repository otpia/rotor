...
nonisolated struct ReorderableStack<Axis: ContainerAxis, Data: RandomAccessCollection, Content: View>: View where Data.Element: Identifiable, Data.Index == Int {
  ...
  private func edgeCheck(_ stackDrag: DragGesture.Value, _ scrollDrag: DragGesture.Value) -> Void {
    ...
    // Rotor 调速：原 speed=3 / 100fps = 300pt/s 偏慢，改为 8 / 100fps = 800pt/s
    let bumperSize = 80.0
    let speed = 8.0
...
@available(iOS 18.0, *)
nonisolated struct ReorderableElement<Position: AxisPosition, Element: Identifiable, Content: View>: View {
  var datum: Element
  var isDragged: Bool
